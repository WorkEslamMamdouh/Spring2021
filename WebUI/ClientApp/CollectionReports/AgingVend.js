...
(function (AgingVend) {
    ...
    function InitalizeComponent() {
        if (SysSession.CurrentEnvironment.ScreenLanguage == "ar") {
            ...
            document.getElementById('Screen_name').innerHTML = "Ages of Vendors indebtedness";
        }
    ...
    function displayvendor() {
        if (txt_ID_APP_Category.value != "Null" && txt_ID_APP_Group.value == "Null") {
            ...
            $('#txt_ID_Vendor').html('');
            $('#txt_ID_Vendor').append('<option value="null"> اختر المورد</option>');
            for (var i = 0; i < Details_Vendornew.length; i++) {
                $('#txt_ID_Vendor').append('<option value="' + Details_Vendornew[i].VendorID + '">' + (lang == "ar" ? Details_Vendornew[i].NAMEA : Details_Vendornew[i].NAMEL) + '</option>');
            }
        }
